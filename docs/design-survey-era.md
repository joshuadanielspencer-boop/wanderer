...
>
> This described the game as a **photo safari in space** — Shutterbug's loop
> relocated to the solar system. The project has since been reframed as
> **SOLBOUND**, an economic strategy game. **`docs/design.md` is the master
> document; where the two disagree, it wins.**
>
> Kept because two things in here are still live and still correct:
> - The **survey layer survives** as the opening hours and as the prospecting
>   mechanic — see design.md §13. You cannot build where you have not surveyed.
> - **§1.5 is a real playtest record** (2026-07-21) and its findings still hold:
>   parallel craft are what rescue the time-vs-fuel tension, light lag bites at
>   the operational scale rather than the interplanetary one, and the terminator
>   is what fuses the calendar to the surface.
>
> Everything here about clue ladders, naming themes and the photo-safari as the
> *core* loop is superseded.
